...
holidays.forEach(holiday => {
  const date = new Date(holiday[0]);
  const month = date.getMonth() + 1;
  const day = date.getDate();
  if (!(month in holidaysObj)) {
    holidaysObj[month] = {};
  }
  if (day in holidaysObj[month]) {
    holidaysObj[month][day].push(holiday[1]);
  } else {
    holidaysObj[month][day] = [holiday[1]]
  }
});
console.log(JSON.stringify(holidaysObj));